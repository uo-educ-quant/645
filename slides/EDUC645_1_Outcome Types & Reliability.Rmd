...
i_am("slides/EDUC645_1_Outcome Types & Reliability.rmd")
...
--
For example, the number of participants who are inactive and have diabetes is N = 211, and summing the row (211 + 1304) gives the total number of inactive participants (N = 1,515). 
...
But what happens if we want to look at the relationship between a *continuous* independent variable, like `Age`, and a dichotomous outcome like `Diabetes`. 
...
--
- There are a few options, but the most commonly used is <span style = "color:green"> logistic regression. </span> The properties of odds (versus probability) make logistic regression models easier to estimate.
...
--
Counts/rates require analyses that account for these properties. These methods are similar to those used for dichotomous outcomes, because another way of thinking of counts is that they measure a series of dichotomous outcomes. For example, the number of times a student is absent (versus present).
...
--
```{r, warning=FALSE, echo=FALSE, fig.retina=3}
NHANES_data1112 %>% 
...
* Individuals with the same true value of the underlying (aka latent) construct - like depression or happiness - have similar responses to the measure.

* Responses to the scale won't systematically differ just because measurement is conducted at different times or by different researchers.

* Scale questions (items) are internally consistent, meaning they measure the same construct.


---
# Common Reliability Methods

* Test-retest method: tests stability over time

* Inter-rater method: tests equivalence among researchers

* Alternative form method: tests whether different forms/orders of a scale function similarly
...
* Internal consistency estimates can be impacted to missing data, the number of items, whether items share the same direction. Having one or more items coded in the opposite direction of other items is a common source of (artificially) low internal consistency.
...
--
The most common internal consistency measure (coefficient) is Alpha; another option is coefficient Omega. Both range from 0-1, with estimates closer to 1 indicating greater internal consistency. Omega uses more realistic assumptions that improve accuracy of estimate. 
