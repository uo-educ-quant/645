...
library(ggeffects)
pop_m3_int_pred <- ggeffect(pop_m3_int_fin,
                             type = "re",
...
--
2) Random-intercepts model including the `treatment x time` interaction and control variables (sex and age). For random-effects, we generally want at least 5 clusters. Here, we only have 2 centers, so we'll instead include a fixed effect for center (i.e., a control variable).
...
--
`ranova()` isn't available for `glmer` models, so we'll just use the likelihood ratio test we used before. This is appropriate because the models differ in only one random slope, not two.
...
```{r}
performance::compare_performance(respire_m1, respire_m2, metrics = "common", verbose = FALSE) %>% 
  print_md()
```
...
```{r}
respire_m1b <- glmer(outcome ~ assignment*visit + sex + age.c + 
                      (1 | id), family = binomial, 
                    control = glmerControl(optimizer = "bobyqa", 
                                           optCtrl = list(maxfun = 2e5)),
                    data = respire)
...
```{r}
# using the ggeffects package
respire_m1b_probs <- ggeffect(respire_m1b,
                             type = "re",
                             terms = c("visit", "assignment"),
                             vcov.fun = "vcovCR", vcov.type = "CR0", 
                             vcov.args = list(cluster = respire$center))
```
