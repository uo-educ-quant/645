...
```{R, setup, include = F}
library(pacman)
p_load(NHANES, here, tidyverse, ggplot2, xaringan, knitr, kableExtra, foreign, broom, xaringanthemer, reshape2, lfe, arsenal, ggpubr, stargazer, fixest, gtsummary, huxtable, aod)

i_am("slides/EDUC645_dichotomous_outcomes.rmd")
...
--
- Ways of representing dichotomous outcomes


--
- Thinking in probability (and odds)
...
- In a dataset, these may be represented as 0 or 1, or with names that correspond to two categories. 
  ...
  + Analytically, named categories are treated just like 0 or 1. 
...
NHANES_data %>% 
  ...
  geom_bar(fill = c("yellow", "royalblue", "darkgray"), width = 0.5, alpha = 0.8) +
...
```{r, echo = F}
NHANES_data %>% 
  filter(PhysActive == "Yes", !is.na(PhysActiveDays)) %>% 
  ggplot(aes(factor(PhysActiveDays))) +
  geom_bar(fill = "royalblue", width = 0.5, alpha = 0.8) +
  labs(x = "Participates in Physical Activities (days per week)",
       y = "Number of Participants")
```
...
$$\frac{Participants\ with\ Diabetes}{All\ Participants} = 0.09\ or\ 9\%$$
...
$$\frac{Participants\ without\ Diabetes}{All\ Participants} = 0.91\ or\ 91\%$$


--
Dividing these two values gives us the *odds* of having diabetes in our data. Odds can have any positive value, and an odds of zero means the outcome didn't occur. 

$$\frac{Probability\ of\ Having\ Diabetes}{Probability\ of\ Not\ Having\ Diabetes} = \frac{0.09}{0.91} = 0.10$$


--
So, in our data, the odds of having diabetes are low (close to zero). This makes sense: only 9% of participants had diabetes. When the probability of the outcome (diabetes) is low (i.e., the outcome isn't very common), probability and odds are similar. As the probability of increases, so do the odds, but odds increase *almost* exponentially as the probability increases.
...
--
- We can calculate the odds just as we did before, but within the physically inactive group (`PhysActive` = No) and the physically active group (`PhysActive` = Yes): 



$$\ Inactive\ Participants: \frac{Probability\ of\ Having\ Diabetes}{Probability\ of\ Not\ Having\ Diabetes} = \frac{0.13}{0.87} = 0.15$$


$$\ Active\ Participants: \frac{Probability\ of\ Having\ Diabetes}{Probability\ of\ Not\ Having\ Diabetes} = \frac{0.06}{0.94} = 0.06$$
...
$$\ OR = \frac{Odds\ of\ Being\ Active\ and\ Having\ Diabetes}{Odds\ of\ Being\ Inactive\ and\ Having\ Diabetes} = \frac{0.06}{0.15} = 0.40$$


--
- So, compared with being inactive, being active is associated with lower odds of having diabetes. Both interpretations are mathematically equivalent. 
...
# inactive participants (top row) and active participants (bottom row)
table(NHANES_data$PhysActive, NHANES_data$Diabetes)
# For example, the number of participants who are inactive and have 
# diabetes is N = 230. Summing the row (230 + 1536) gives the total 
# number of inactive participants (N = 1766). 

# Calculate the probability of having diabetes for inactive 
# participants (top row), rounding to 2 decimal places:
round((230 / (230 + 1536)), digits = 2)

# Then, probability of NOT having diabetes for inactive participants:
round((1536 / (230 + 1536)), digits = 2) # Equal to 1 - 0.13.
```
...
```{r, echo = TRUE}
# Calculate the odds of having diabetes for inactive participants 
# using those results:
round((0.13 / (1 - 0.13)), digits = 2)

# Do the same for active participants (bottom row):
round((140 / (140 + 2174)), digits = 2)
round((0.06 / (1 - 0.06)), digits = 2)

# Finally, divide the two odds to get the odds ratio:
round((0.15 / 0.06), digits = 2)
```
