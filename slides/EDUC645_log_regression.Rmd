...
- Compare the unadjusted regression coefficients to the adjusted regression coefficients (not the ORs)
- 10% confounding rule: if the change in coefficients > 10%, there is evidence for confounding by the variable in the adjusted model
...
* In logistic regression, use the predicted probabilities from the model to classify people according to one of the two groups of the predictor variable.
...
* We can calculate a cut-off by examining the *sensitivity* and *specificity* of our model
...
* _Negative predictive value (PPV)_ = $\frac{number of True -} {Total_{observed} Y = 0}$
...
* For more accurate tests, the curve is closer to the left-hand border and the top border of the ROC
* The __area under the curve (AUC)__ measures *accuracy*

---
# Area Under the ROC Curve (AUC)

[insert figure of theoretical AUC]
