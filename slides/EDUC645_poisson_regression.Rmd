...
Research inquiry: Our primary goal is to investigate the school-level factors associated with teacher job satisfaction that is measured by the count of teachers who feel satisfied with their job and their school.  

The data: Drawn from the China Education Panel Survey (CEPS), China’s first nationally representative, longitudinal survey of middle school students. This school-level dataset includes 45 schools, each with eight teacher participants.
...
OLS regression: Count outcome variables are sometimes log-transformed and analyzed using OLS regression. 
...
Negative binomial regression: For over-dispersed count data, when the conditional variance exceeds the conditional mean. 
- A generalization of Poisson regression with the same mean structure, but an extra parameter to model the over-dispersion. 
- If the conditional distribution of the outcome variable is over-dispersed, the CIs for coefficients are likely to have a wider range a Poisson regression.

Zero-inflated regression model: Attempt to account for excess zeros
- When researcher suspects that there are two kinds of zeros in the data, *true* and *excess* zeros
- Estimate two equations simultaneously, one for the count model (with *true* zeros) and one for the *excess* zeros

---
# Why OLS may not be ideal for count data
...
---
# Limitations to Applying OLS to Count Data

An important reason why a linear least squares regression model may not be preferred for Poisson data is the violation of equal variance assumption:
...
- $\beta_1$: natural log of the rate ratio between a given group and the reference group
	...
	- Represents the *change in response corresponding to a one unit difference in the corresponding predictor.* 
