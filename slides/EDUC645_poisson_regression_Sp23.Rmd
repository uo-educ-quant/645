...
We can then use the same tools to assess model assumptions as we used in logistic regression. For large datasets like ours, an outlier test can be oversensitive (it may be significant when there are actually not concerning outliers). Increasing the number of simulations in the DHARMa function helps to address this issue (type 1 error).
...
```{r}
performance::check_model(mod_3_int, check = c("vif"))
```

---
# Assumptions and diagnostics

Checking multicollinearity using the `vif()` function from the *car* package. 

```{r, warning=FALSE}
car::vif(mod_3_int, type = "predictor")
```
...
- Assignment 2 due May 11 (previously May 9) - Available after class today.
